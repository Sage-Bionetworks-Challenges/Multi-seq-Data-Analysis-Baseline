...
function (count_path, infile = "csv", outfile = "csv", type = "count", out_dir, labeled = FALSE, 
          drop_thre = 0.5, Kcluster = NULL, labels = NULL, genelen = NULL, ncores = 5, file_name) # Edits: add file_name
{   
    ...
    # Edits: hide warnings to prevent from crash
    ...
    # Edits: using fwrite and use custom output file name instead
    # write_count(count_imp, filetype = outfile, out_dir, type = type, genelen = genelen)
    write_count(count_imp, filetype = outfile, out_dir, type = type, genelen = genelen, file_name = file_name)
